...
function seedInquiries(db: ReturnType<typeof getDB>): number {
  const existing = (db.prepare('SELECT COUNT(*) as cnt FROM inquiries').get() as { cnt: number }).cnt
  ...
  for (const inq of inquiries) insert.run(inq)
  return inquiries.length
}

function seedProjects(db: ReturnType<typeof getDB>): number {
  const existing = (db.prepare('SELECT COUNT(*) as cnt FROM projects').get() as { cnt: number }).cnt
  if (existing > 0) return 0
  ...
  for (const proj of projects) insert.run(proj)
  return projects.length
}

function seedEvents(db: ReturnType<typeof getDB>): number {
  const existing = (db.prepare('SELECT COUNT(*) as cnt FROM events').get() as { cnt: number }).cnt
  if (existing > 0) return 0
...
function seedEmails(db: ReturnType<typeof getDB>): number {
  const existing = (db.prepare('SELECT COUNT(*) as cnt FROM emails').get() as { cnt: number }).cnt
  if (existing > 0) return 0
...
async function main() {
  ...
  const inquiryCount = seedInquiries(db)
  console.log(`Inquiries: ${inquiryCount > 0 ? `${inquiryCount} seeded` : 'already seeded, skipped'}`)

  const projectCount = seedProjects(db)
  console.log(`Projects: ${projectCount > 0 ? `${projectCount} seeded` : 'already seeded, skipped'}`)

  const eventCount = seedEvents(db)
  console.log(`Events: ${eventCount > 0 ? `${eventCount} seeded` : 'already seeded, skipped'}`)

  const emailCount = seedEmails(db)
  console.log(`Emails: ${emailCount > 0 ? `${emailCount} seeded` : 'already seeded, skipped'}`)
